...
The joint probability function of two discrete random variables $X$ and $Y$ is given by $P(X=x, Y=y)$ and describes the probability that $X=x$ and $Y=y$ simultaneously. In our case $X$ is the home team and $Y$ is the away team, and the amount of goals they score we are treating as independent for this part with additional weighting to be applied with the use of other statistical techniques. For the home team, $x$ takes the values in its range $R_{x}$, obtained from the past data of what values of x the team has obtained prior. The away team follows the same strategy.
...
\end{center}
 ...
 The Kolmogorov-Smirnov test is a non-parametric statistical test that compares the empirical distribution of a data sample with a known probability distribution. In our case, we used the Kolmogorov-Smirnov test to compare the empirical distribution of the observed data with the Poisson distribution.
...
# Read in data
data <- read.csv("C:\\Users\\Tom\\OneDrive - University of Manitoba\\Desktop\\World Football Elo Rankings.csv", header = FALSE)
...
\quad We repeat the same process for Czechoslovakia. (Appendinx 2.12)
...
When Brazil plays Czechoslovakia , the new $\lambda$ value is `r new_brazil` , which has increased from `r lambda_Brazil`. On the flip side, when Czechoslovakia plays Brazil and wins, the new $\lambda$ value is `r new_czechoslovakia` decreased from `r lambda_czechoslovakia` meaning they are less likely to score since they are playing against a better team.
...
poisson_calc_final <- function(team1,team2,team1_lambda_pr,team2_lambda_pr){
  ...
  prob_matrix <- outer(dpois(sort(team2_gs),team2_lambda_pr),dpois(sort(team1_gs),team1_lambda_pr), "*")
  prob_table <- prop.table(prob_matrix)
  prob_matrix[is.na(prob_matrix)] <- default_val
  prob_matrix
...
table_elo <- poisson_calc_final("Brazil","Czechoslovakia",2.195533,1.170391)
...
print(xtable(table_elo), type = "latex", comment = FALSE, include.rownames = TRUE)
```
\newpage
